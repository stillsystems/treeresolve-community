...
* **Pre-Publish Automated Guardrails**: Packaging scripts invoke `tools/check-no-test-checkout-links.js` during `npm run prepublishOnly` to ensure zero Paddle sandbox checkout URLs (`sandbox-buy.paddle.com`) reach release packages.
...
* **Isolation**: This fingerprint is transmitted solely to the configured licensing gateway (`https://treeresolve-licensing.still-systems.workers.dev` today; launch target `https://licensing.stillsystems.com`) for lease validation and is never correlated with telemetry metrics, source code, or repository contents. Offline wildcard licenses never contact the network.
